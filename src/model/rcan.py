...
class RCAN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        # define tail module
        # # defult, n_feats-->n_feats*2^3-->upscale-->n_feats*2^3-->upscale-->n_colors(3)
        modules_tail = [
            common.Upsampler(conv, scale, n_feats, act=False),
            conv(n_feats, args.n_colors, kernel_size),
        ]
        # # direct upsample, n_feats-->n_colors*4^3-->upscale
        # modules_tail = [
        #     conv(n_feats, args.n_colors*scale**3, kernel_size),
        #     common.PixelShuffle3d(scale)]
        # # UpConv, upsacle-->n_feats-->n_feats-->n_colors
        # modules_tail = [
        #     common.Upconv(scale, n_feats, act=False),  # act=False, 'leakyrelu'
        #     conv(n_feats, args.n_colors, kernel_size)]
